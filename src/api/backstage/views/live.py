...
class LiveViewList(BackstageBaseAPIView):
    ...
    def __init__(self, *args, **kwargs):
        super(LiveViewList, self).__init__(*args, **kwargs)
        self.col = self.db.live
    ...
    def get(self, request):
        ...
        msg = u"LiveView:get user:%s, request.data is:%s" % (
        ...
        equal_key = ['content_text', 'machine_tags']
        ...
        if is_page == NEED_PAGE:
            pos = (index-1) * number
            # all_live_lst = self.col.find(new_query_dict).hint('123')
            if descent == 'craw_time':
                all_live_lst = self.col.find(new_query_dict).hint('live_top_time_desc')
            if descent == 'click_count':
                all_live_lst = self.col.find(new_query_dict).hint('live_top_click_desc')
            else:
                all_live_lst = self.col.find(new_query_dict).hint('live_top_pub_desc')
            if pos > 0:
                live_lst = all_live_lst.skip(pos).limit(number)
            else:
                live_lst = all_live_lst.limit(number)
            llen = all_live_lst.count()
            num_pages = llen / number
            if llen % number != 0:
                num_pages = num_pages+1
            logger.info(u'使用分页方式正常获取直播信息')

            return BackstageHTTPResponse(BackstageHTTPResponse.API_HTTP_CODE_NORMAL,
                                   data=self.get_serializers_mongo(live_lst),
            ...
            all_live_lst = self.col.find(new_query_dict).sort(sort_tuple)

            logger.info(u'正常获取直播信息')
            return BackstageHTTPResponse(BackstageHTTPResponse.API_HTTP_CODE_NORMAL,
                                        data=self.get_serializers_mongo(all_live_lst),).to_response()
    ...
    def put(self, request):
        ...
        msg = u"LiveView:put request.data is:%s" % str(post_data)
        logger.info(msg)
        ...
        if is_rmd and is_rmd not in ['0', '1']:
            ...
            vals['is_rmd'] = is_rmd
        res = []
        # selfcol = self.db.live
        if vals:
            for i in id_list:
                _id = get_mongo_id(i)
                live_obj = self.col.find_one({'_id': _id})
                if not live_obj:
                    logger.info(u"LiveView:id %s ID对应的消息不存在",
                                _id)
                    ...
                    live_obj = self.col.find_one({'_id': _id})
                    logger.info(u'通过主键修改live成功')
                    live_obj['_id'] = str(live_obj['_id'])
                    res.append(live_obj)
    ...
    def delete(self, request):
        ...
        msg = u"LiveView:delete request.data is:%s" % str(post_data)
        logger.info(msg)
        id_list = post_data.get('id_lists', '')
        if not id_list:
            return BackstageHTTPResponse(BackstageHTTPResponse.API_HTTP_CODE_INVILID_PARAMS,
    ...
    def post(self, request):
        ...
        logger.info(u'添加一篇直播%s' %post_data)
        ...
        vals['content_html'] = post_data.get('content_text')
        ...
        vals['_id'] = _id
        logger.info(u'成功添加一篇直播')
        return BackstageHTTPResponse(data=vals,
                                     message=u'成功添加一篇直播').to_response()


class LiveView(BackstageBaseAPIView):

    def __init__(self, *args, **kwargs):
        super(LiveView, self).__init__(*args, **kwargs)
        self.col = self.db.live
    ...
    def get(self, request, pk):
        ...
        if live_obj:
            old_click = live_obj.get('click_count', 0)
            self.col.update({'_id': _id}, {'$set': {'click_count': int(old_click)+1}})

            return BackstageHTTPResponse(BackstageHTTPResponse.API_HTTP_CODE_NORMAL,
                               data=live_obj).to_response()
